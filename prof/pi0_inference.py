...
def _make_dummy_batch(policy_cfg: PreTrainedConfig, device: str) -> dict[str, torch.Tensor]:
    ...
    batch[OBS_LANGUAGE_ATTENTION_MASK] = torch.ones((1, max_len), dtype=torch.bool, device=device)
    return batch
...
def main() -> None:
    ...
    if args.use_dummy_input:
        sample = _make_dummy_batch(policy_cfg, device)
        preprocessor = None
